...
export default function JadwalSolatHariIni() {
  ...
  setInterval(() => {
    ...
    if (jadwalSholat) {
      if (jadwalSholat.timings) {
        ...
        setNext({
          name: times[0][0],
          countDown: `${(hours < 10 ? "0" : "") + hours}:${(minutes < 10 ? "0" : "") + minutes}:${(seconds < 10 ? "0" : "") + seconds}`,
        });
      }
    }
  }, 1000);
  ...
  return (
    <Layout name="Jadwal Sholat">
      ...
      <div className={`fixed inset-0 p-3 bg-white duration-300 ${displayMap ? "visible" : "invisible"}`}>
        ...
        <button onClick={() => setDisplayMap(!displayMap)} className="px-3 py-2 rounded-lg bg-rose-500 text-rose-50">
          Simpan
        </button>
      </div>
        ...
        <>
          {jadwalSholat.date && (
            <>
              ...
              <button onClick={() => setDisplayMap(!displayMap)} className="px-3 py-2 mt-3 rounded-lg bg-rose-500 text-rose-50" title="Klik untuk mengatur lokasi sesuai keinginan">
                Atur lokasi
              </button>

              <div className="grid grid-cols-2">
                <p className="flex items-center mt-3">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="" className="mr-2" viewBox="0 0 16 16">
                ...
                <p className="flex items-center mt-3">
                  ...
                  {jadwalSholat.date.hijri.day} {jadwalSholat.date.hijri.month.en} {jadwalSholat.date.hijri.year}
                </p>
                <p className="flex items-center mt-3">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="mr-2 ml-0.5" viewBox="0 0 16 16">
                ...
                <p className="flex items-center mt-3">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="mr-3" viewBox="0 0 16 16">
                ...
                <p className="my-3 col-span-2">
                  Berikutnya <strong>{next.countDown}</strong> lagi menuju <strong>{indonesianName(next.name)}</strong>
                </p>
              </div>
            </>
          )}
      ...
      <audio id="adzan" controls src="/adzan.mp3" className="hidden" loading="lazy" />
    </Layout>
  );
}
